    ...
    <Screen padTop={0}>
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, padding: '50px 18px 12px', background: '#fff', borderBottom: `1px solid ${X.LINE}`, zIndex: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <button onClick={() => router.push('/helper/pickup-aed')} aria-label="Back to AED route" style={{ all: 'unset', cursor: 'pointer', width: 36, height: 36, borderRadius: 12, background: X.INK, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Icon name="chevron-right" size={20} color="#fff" stroke={2.4} style={{ transform: 'rotate(180deg)' }}/>
          </button>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 11, fontFamily: FONT.mono, letterSpacing: 1, color: X.RED, fontWeight: 700 }}>● DIRECT TO PATIENT · AED SKIPPED</div>
            <div style={{ fontSize: 16, fontWeight: 700 }}>Eleanor T. · 67 · pacemaker</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ position: 'absolute', left: 12, right: 12, bottom: 28, padding: 16, background: '#fff', borderRadius: 22, boxShadow: '0 14px 40px rgba(0,0,0,0.18)', border: `1px solid ${X.LINE}` }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: 11, color: X.INK2, fontFamily: FONT.mono, letterSpacing: 1 }}>NEXT TURN · 60 m</div>
            <div style={{ fontSize: 20, fontWeight: 700, marginTop: 2 }}>Left onto 4th Street</div>
          </div>
        ...
        <div style={{ marginTop: 12, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, paddingTop: 12, borderTop: `1px solid ${X.LINE}` }}>
          {[['ETA', '1:50', X.RED], ['DIST', '220m', X.INK], ['OTHERS', '+2', X.BLUE]].map(([l, v, c], i) => (
            <div key={i}>
        ...
        <div style={{ marginTop: 10, display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: 10, color: X.INK2, fontFamily: FONT.mono, letterSpacing: 0.6 }}>· tap ☎ to call Eleanor — Sarah still bringing AED</span>
        </div>
        ...
        <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
          <button onClick={() => router.push('/sos/cpr/assist')} style={{ all: 'unset', cursor: 'pointer', flex: 1, padding: 12, textAlign: 'center', background: X.RED, color: '#fff', borderRadius: 12, fontWeight: 700, fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            <Icon name="check" size={16} color="#fff" stroke={2.4}/> I&apos;ve arrived · Start CPR
          </button>
          <button onClick={openCall} aria-label="Open group call" style={{ all: 'unset', cursor: 'pointer', position: 'relative', width: 44, height: 44, background: X.INK, color: '#fff', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
